...
and load the shared security-policy owners it names. This compatibility entry
uses the same action boundary as `ai-tester`.
...
Apply [AI action boundaries](../ai-tester/references/action-boundaries.md):
the live, account, and applicable class policies own approval and stop decisions.
