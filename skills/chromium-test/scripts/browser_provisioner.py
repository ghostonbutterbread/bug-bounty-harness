...
def start(args):
 ...
 got=lease(args,'acquire',args.program,args.account,*(['--auth-domain',args.auth_domain] if args.auth_domain else []),'--agent-id',args.agent_id,'--run-id',args.run_id,'--purpose',args.purpose,'--ttl-seconds',str(args.ttl_seconds),*(['--recover-profile'] if getattr(args,'recover_profile',False) else []))
...
def request(args):
 ...
 while True:
  ...
  if args.kasmvnc_display is not None: cmd += ['--kasmvnc-display',str(args.kasmvnc_display)]
  if args.kasmvnc_web_port is not None: cmd += ['--kasmvnc-web-port',str(args.kasmvnc_web_port)]
  if getattr(args,'recover_profile',False): cmd += ['--recover-profile']
