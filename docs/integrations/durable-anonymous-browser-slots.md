...
- **Review / merge decision:** Self-review reconciled the feature diff against the lease/provisioner contract; it preserves exclusive ownership, persistent profile paths, task-MITM launch routing, and zero account/auth-seed lookup for `anon*`. Accepted for local beta integration.
- **Next:** Merge into a clean current `beta`, rerun the focused tests from beta, then deploy the reviewed beta artifact to the desired browser node.
