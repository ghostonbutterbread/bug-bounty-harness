...
class SingleAgentTeam(BaseTeam):
    ...
    def _single_agent_prompt_template(self) -> str:
        ...
        fresh_line = "- Treat prior coverage as advisory only; start from the supplied goal and current target state." if self.fresh else "- Use prior ledger and coverage only as targeted coordination inputs after selecting the live surface or before adding findings; confirmed, submitted, and duplicate findings are closed to default work selection unless the user explicitly asks to retest, report on, or use them as inspiration."
