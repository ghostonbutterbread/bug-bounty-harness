...
COOKIE = {'name': '__Host-session', 'domain': 'localhost', 'path': '/',
          'httpOnly': True, 'secure': True}
OBSERVATION = {'url': ORIGIN + '/me', 'status': 200, 'redirected': False, 'principal': 'anon'}


@pytest.mark.parametrize('fault', ['begin-lost', 'evaluate-failed', 'end-lost', 'begin-unproved', 'end-unproved',
                                  'wrong-owner', 'wrong-generation', 'wrong-principal', 'redirect',
                                  'wrong-cookie', 'wrong-origin'])
def test_promotion_probe_exact_release(tmp_path, monkeypatch, fault):
    ...
    monkeypatch.setattr(manager, 'STATE', tmp_path / 'state' / 'manager.json')
    manager.STATE.parent.mkdir(mode=0o700)
    ...
    class Client:
        ...
        def post(self, url, json):
            ...
            if action == 'call':
                ...
                if json['method'] == 'Network.getAllCookies':
                    return Response({'result': {'cookies': [{**COOKIE, 'secure': False}]
                                     if fault == 'wrong-cookie' else [COOKIE]}})
                expression = json['params']['expression']
                value = (ORIGIN if expression == 'location.origin' else 'approved'
                         if 'localStorage.getItem' in expression else OBSERVATION)
                if fault == 'wrong-origin' and expression == 'location.origin':
                    value = 'http://localhost:31338'
                if fault == 'wrong-principal' and isinstance(value, dict):
                    value = {**value, 'principal': 'other'}
                if fault == 'redirect' and isinstance(value, dict):
                    value = {**value, 'redirected': True, 'status': 302}
    ...
    class Client:
        ...
        def post(self, url, json):
            ...
            if action == 'call':
                ...
                if json['method'] == 'Network.getAllCookies':
                    return Response({'result': {'cookies': [COOKIE]}})
                expression = json['params']['expression']
                value = (ORIGIN if expression == 'location.origin' else 'approved'
                         if 'localStorage.getItem' in expression else OBSERVATION)
                return Response({'result': {'result': {'value': value}}})
