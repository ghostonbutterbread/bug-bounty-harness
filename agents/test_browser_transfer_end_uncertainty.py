    ...
    class Client:
        ...
        def post(self, url, json):
            ...
            if method == 'Network.getAllCookies':
                value = [{'domain': 'localhost', 'name': '__Host-session', 'path': '/',
            ...
            if method == 'Runtime.evaluate':
                ...
                elif expression.startswith('(async () =>'): value = {'url': origin + '/me',
                    'status': 200 if s['cookie'] and s['storage'] else 401,
                    'redirected': False, 'principal': 'anon' if s['cookie'] and s['storage'] else 'guest'}
