#!/usr/bin/env python3
"""Regression coverage for the manual-places-to-hunt skill contract."""
...
class ManualPlacesToHuntSkillTests(unittest.TestCase):
    def test_skill_is_registered_and_read_only_by_default(self) -> None:
        skill_path = ROOT / "skills" / "manual-places-to-hunt" / "SKILL.md"
        text = skill_path.read_text()
        registry = (ROOT / "SKILL_REGISTRY.md").read_text()

        self.assertIn("name: manual-places-to-hunt", text)
        ...
        self.assertIn("manual-places-to-hunt", registry)
    ...
    def test_skill_preserves_canonical_owners_and_live_action_gate(self) -> None:
        text = (ROOT / "skills" / "manual-places-to-hunt" / "SKILL.md").read_text()
