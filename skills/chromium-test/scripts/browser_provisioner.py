...
def start(args):
 ...
 if getattr(args,'headless',False): cmd += ['--headless']
 if getattr(args,'auth_seed_file',None): cmd += ['--auth-seed-file',args.auth_seed_file]
 ...
 deadline=time.time()+float(os.environ.get('BROWSER_PROVISIONER_LAUNCH_WAIT','8'))
