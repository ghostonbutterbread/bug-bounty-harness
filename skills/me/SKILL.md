...
- Manual Hunter requires an explicit `--lane`; it has no implicit destination default.
- `--lane web|api` routes web/API targets; `--lane apk|exe|mac` routes binary targets.
- `--hunt-type` is legacy metadata only and never overrides the explicit lane.
...
- Raw manual report inputs belong in `reports/raw/{type}/` or should be imported with `manual_hunter.py --lane <web|api|apk|exe|mac> --from-file`.
