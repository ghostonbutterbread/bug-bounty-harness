...
> published Bounty Core revision pinned in the canonical `requirements.txt`,
> alongside BBH runtime and test dependencies, into that checkout's own `.venv` using
...
> BBH does not use editable installs or clone Bounty Core during setup.
