...
def test_stale_source_owner_and_recipient_decline(tmp_path, monkeypatch):
    ...
    assert manager.manager_fixture_apply('destination', generation=1, owner_agent_id='wrong', owner_run_id='run', approved_boundary=True)['reason'] == 'pending-peer-unavailable'
    with lease.connect(canonical) as store:
