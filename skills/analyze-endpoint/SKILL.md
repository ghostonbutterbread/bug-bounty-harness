...
4. If a raw request block or file is available, initialize artifacts:
   ```bash
   python3 "$HARNESS_ROOT/skills/analyze-endpoint/scripts/analyze_endpoint.py" <program> request.raw
   ```
5. Read `$HARNESS_ROOT/prompts/analyze-endpoint-playbook.md` for the full workflow.
6. Read `references/artifact-contract.md` for required artifact fields and merge rules.

## Storage

Write endpoint analysis under:

```text
/home/ryushe/Shared/bounty_recon/{program}/ghost/endpoints/{host}/{method}_{route_slug}_{hash}/
  contract.json
  parameters.json
  replay.md
  observations.jsonl
...
For web-lane work that already uses `/home/ryushe/Shared/web_bounty/{program}`, mirror or link the final folder from the active lane notes, but keep this skill's canonical contract shape.
