...
DATE_SHAPED_DIR_RE = re.compile(r"^\d{4}-\d{2}-\d{2}$")
...
def status_index_paths_for_read(reports_root: Path) -> list[Path]:
    if not reports_root.is_dir():
        return []
    return [
        path
        for bucket, legacy_filename in STATUS_REPORT_SPECS
        if (path := status_report_path_for_read(reports_root, bucket, legacy_filename)) is not None
    ]


def has_status_index_markdown(reports_root: Path) -> bool:
    return bool(status_index_paths_for_read(reports_root))
...
def report_index_roots_for_read(
    ...
    *,
    ...
    if has_status_index_markdown(canonical_root):
    ...
    return [
        source
        for source in legacy_ghost_report_sources(program, hunt_type)
        if has_status_index_markdown(source.path)
    ]
