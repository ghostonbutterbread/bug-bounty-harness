...
Launch Claude Code workers with the installed role-specific Agent `subagent_type`: `bunny-hunter`, `bunny-recon`, `bunny-verifier`, or `bunny-reporter`. These definitions ship in [`agents/`](agents/) and must be projected into Claude Code's agent discovery root before dispatch; a role written only in the prompt or `description` still displays as `general-purpose`. On each Claude host, link each bundled `agents/bunny-*.md` file from the runtime-resolved Bunny skill directory into `~/.claude/agents/` (or the running project's `.claude/agents/`); inspect existing entries and never overwrite unrelated agent definitions. Start a fresh Claude session and confirm it can invoke one of the role types; an existing session may not reload newly linked definitions. Put the leased surface and run ID in the ordinary scoped `description` and full task packet, so same-role workers remain distinguishable without changing their role. If a role type is unavailable, report the missing projection instead of silently launching `general-purpose`. These labels confer no authority or proof of progress; inspect actual status and evidence before reporting either.
