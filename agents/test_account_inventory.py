...
def test_primary_idor_accounts_are_stored_and_visible_in_human_inventory_show(tmp_path, monkeypatch, capsys):
    module = load_inventory_module()
    monkeypatch.setenv("HARNESS_SHARED_BASE", str(tmp_path / "shared"))

    assert module.main(["add-account", "demo", "--alias", "pink"]) == 0
    assert module.main(["set-primary-idor-accounts", "demo", "--account", "pink"]) == 0
    assert module.main(["show", "demo"]) == 0

    output = capsys.readouterr().out
    assert "Primary IDOR accounts: pink" in output
