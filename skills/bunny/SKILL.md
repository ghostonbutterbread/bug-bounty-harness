...
Name each launched worker for its **actual role and leased surface** so the agent manager shows useful identities instead of several indistinguishable `general-purpose` workers. For Claude Code's Agent tool, supply a unique, short `name` such as `bunny-hunter-pg-extensions`, `bunny-recon-compute`, `bunny-verifier-jwks`, or `bunny-reporter-d12`, alongside the ordinary scoped `description` and full task packet. Use the tool's supported name field (or a role-named agent type in a runtime that lacks it); a role written only inside the prompt does not rename the displayed agent. Keep names unique within the run, including when two hunters share a role, and update the name when assigning a genuinely different role. Names are operational labels, not authority or proof of task progress; inspect the actual agent status and evidence before reporting either.
